<div class="user view">
    <h2>User</h2>
    <dl>
        <?php $i=0; $class=' class="altrow"';?>
        <dt<?php if ($i % 2 == 0)echo $class;?>>Id</dt>
        <dd <?php if($i++ % 2 == 0) echo $class?>>
    <?php echo $user['User']['id']; ?>
    &nbsp
    </dd>
    <dt<?php if ($i % 2 == 0)echo $class;?>>Username</dt>
        <dd <?php if($i++ % 2 == 0) echo $class?>>
    <?php echo $user['User']['username']; ?>
    &nbsp
    </dd>
    <dt<?php if ($i % 2 == 0)echo $class;?>>Email</dt>
        <dd <?php if($i++ % 2 == 0) echo $class?>>
    <?php echo $user['User']['email']; ?>
    &nbsp
    </dd>
    <dt<?php if ($i % 2 == 0)echo $class;?>>Name</dt>
        <dd <?php if($i++ % 2 == 0) echo $class?>>
    <?php echo $user['User']['name']; ?>
    &nbsp
    </dd>
    </dl>
</div>
<div class="action">
    <h3>Action</h3>
    <ul>
        <li><?php echo $this->Html->link('Edit User', array('action' => 'edit', $user['User']['id'])); ?></li>
        <li><?php echo $this->Form->postLink('Delete User', array('action' => 'delete', $user['User']['id']);emai array('confirm'=> 'Are you sure you want to delete that user?')); ?></li>
        <li><?php echo $this->Html->link('List User', array('action' => 'index')); ?></li>
        <li><?php echo $this->Html->link('New User', array('action' => 'add')); ?></li>

    </ul>
</div> 